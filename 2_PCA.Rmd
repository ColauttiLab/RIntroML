...
If we multiply each loading by its corresponding feature vector, and then sum, we get the PC axis, which we can confirm by plotting:

```{r}
testDat<-Flwr %>%
  mutate(PCcalc=Flwr07*Load[1] + 
           Flwr08*Load[2] +
           Flwr09*Load[3] +
           Flwr10*Load[4])
